...
summary = tasks_data.groupby(['Task ID', 'Method']).agg({'Time': ['mean', 'std']}).reset_index()
...
ax.set_title('Mean Time by Task and Method')
...
ax.annotate(f'p = {p_values[1]:.6f} < 0.05',
                xy=(index[0] + bar_width / 2, 32),
                xytext=(0, 10),  # 10 points vertical offset
                textcoords="offset points",
                ha='center', va='bottom')

ax.annotate(f'p = {p_values[2]:.6f} < 0.05',
                xy=(index[1] + bar_width / 2, 49),
                xytext=(0, 10),  # 10 points vertical offset
                textcoords="offset points",
                ha='center', va='bottom')

ax.annotate(f'p = {p_values[3]:.6f} < 0.05',
                xy=(index[2] + bar_width / 2, 54.5),
                xytext=(0, 10),  # 10 points vertical offset
                textcoords="offset points",
                ha='center', va='bottom')
